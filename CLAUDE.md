...
- **Lights redesign (2026-09-19, PU.luci)**: `lights/` (own process, sound-reactive, independent of video; new hotkeys `PUPA_LUCI`/`PUPA_LUCI_LIVELLO_1..3`) is built and offline-tested but NOT yet deployed/live; `pupa.py` still holds the OLD light code (below) until phase 2 removes it - never run both against the same QLC+. Details/checklist in `LIGHTS_CONFIG.md`. The bullet below describes the OLD behaviour.
...
- OBS hotkeys are not a native WebSocket concept — PUPA polls the visibility of dummy sources in a hidden `PUPA_Control` scene (`hotkey_controller.py`'s `MultiLevelControl`/`BinaryControl`). Current bindings: F5/F6/F7 light mode, F8 manual white strobe, F9/F10 solo-monitor/solo-luci override, F11 blackout, plus the older CALM MODE (4 levels) and loop-scene toggle. See `OBS_CONFIG.md`'s "Hotkey control sources" table for the full list and how to add a new one.
