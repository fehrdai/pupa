...
- **`LIGHTS_CONFIG.md`** — the QLC+/lighting side: OS2L channel map per fixture, the single-VC-page requirement, Web API setup for diagnostics. **Lights are being redesigned (2026-09-19): new independent process `lights/pupa_luci.py`, offline-validated only — read its STATO ATTUALE first.**
...
- **Lights (2026-09-19, PU.luci)**: the lights are a SEPARATE process, `lights/pupa_luci.py` (sound-reactive; only link with the video = the colour: it follows PUPA live's identity colour via `logs/identity_color.json` written by `pupa.py`; ping-pong on kicks, no automatic strobe, own hotkeys `PUPA_LUCI`/`PUPA_LUCI_LIVELLO_1..3` + F8/F11/F12). `pupa.py`/`brain.py` no longer contain any light code (removed, commit `b5b3aaa`). Start it on Linux next to `pupa.py`: `python3 lights/pupa_luci.py`. Details, tests, the QLC+ OS2L message-loss finding (pace messages >=1 ms!) in `LIGHTS_CONFIG.md`.
- `qlc_controller.py` (OS2L, TCP 9996) is transport only, used by `lights/`; QLC+ silently drops OS2L messages sent back-to-back, so callers must space them (see `LIGHTS_CONFIG.md`).
- Identity colors are pure RGB primaries only (red/green/blue) - `yellow_color` was removed entirely (both the OBS scenes and all code references), never re-add it as a rotating identity. The lights use the same rule (own palette, `lights/lights_config.py`).
...
- **CALM is a hard override, not a hint** (2026-09-18): CALM 0 and 3 are tuned extremes that force behavior regardless of audio (3 = no cuts/flashes/bursts, slow overlaps, A/B min dwell, kick scenes rare); 1 and 2 are interpolated and untested. Behavior that lives in `pupa.py` (e.g. the kick color-overlay pulse) must go through `brain.get_calm_value()` too, or it will ignore CALM. Details in `OBS_CONFIG.md` and `PUPA_DEVELOPMENT_LOG.md` (2026-09-18).
- **OBS decodes every video source permanently, on air or not** (measured 2026-09-20): each extra video costs CPU whether shown or not, and 15 videos saturated the 4-core Linux rig (choppy video). `media_guard.py` pauses the CALM-exclusive B videos (`calm_scenes` in the machine-specific `scenes_config.yaml`) until needed; use PAUSE/PLAY, never STOP (STOP+PLAY shows a black picture). Details in `OBS_CONFIG.md`.
- OBS hotkeys are not a native WebSocket concept — PUPA polls the visibility of dummy sources in a hidden `PUPA_Control` scene (`hotkey_controller.py`'s `MultiLevelControl`/`BinaryControl`). Current bindings: F8 manual white strobe (lights), F9/F10 solo-luci/solo-monitor override (monitors only now), F11 blackout, F12 shutdown (both processes), F5/Shift+F5 = lights on/off, Shift+1/2/3 = lights level, plus the older CALM MODE (4 levels) and loop-scene toggle. See `OBS_CONFIG.md`'s "Hotkey control sources" table for the full list and how to add a new one.
