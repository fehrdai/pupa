...
- **Lights (2026-09-19, PU.luci)**: the lights are a SEPARATE process, `lights/pupa_luci.py` (sound-reactive, independent of the video, one colour at a time red/green/blue, ping-pong on kicks, own hotkeys `PUPA_LUCI`/`PUPA_LUCI_LIVELLO_1..3` + F8/F11/F12). `pupa.py`/`brain.py` no longer contain any light code (removed, commit `b5b3aaa`). Start it on Linux next to `pupa.py`: `python3 lights/pupa_luci.py`. Details, tests, the QLC+ OS2L message-loss finding (pace messages >=1 ms!) in `LIGHTS_CONFIG.md`.
