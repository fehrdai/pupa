...
**The OLD light code has been REMOVED from `pupa.py` and `brain.py`** (commit `b5b3aaa`, deployed to Linux the same night, backups `pupa.py.bak_2026-09-19_pre-luci-removal` / `brain.py.bak_...` on Linux): `pupa.py` no longer talks to QLC+ at all. Verified: pyflakes clean, brain simulation identical decision-for-decision to the original on the same seeded input, `pupa.py` + `pupa_luci.py` running together on the rig with music (CALM 3, no errors, no `[QLC]` lines from `pupa.py`). Left in place on purpose: `brain.forced_mode` (F9/F10 still black/force the MONITORS), F11 (monitors), F12, the unused strobe getters in `brain.py`, and the now-inert OBS sources `PUPA_LIGHTMODE_*` (F5-F7). `qlc_controller.py` still has no message pacing (only `lights/` and `shutdown_helpers` pace) - `exhibition/`/`slideshow/` would inherit the loss if they ever send bursts.
...
| Colour | **Follows PUPA live (2026-09-19 night, operator): the lights take the current identity colour of the video - all red, all green or all blue, both fixtures the same** (`FOLLOW_VIDEO_COLOR`). One-way link, the ONLY coupling with the video: `pupa.py` writes `logs/identity_color.json` (`{"color": "red_color", "t": ...}`) on every identity change and as a 2 s heartbeat (`_publish_identity_color`, best effort, wrapped so it can never touch the video loop); `pupa_luci.py` reads it every 0.5 s. If the file is missing or older than 10 s (`pupa.py` not running) or the colour isn't a primary, the lights fall back to their own rotation (single colour rotating per phrase, `SINGLE_COLOR`). The swap happens when the fixture is dark (invisible). No yellow; white reserved for the manual strobe. |
| Break | `is_break` → kicks ignored, slow breathing (8 s period, 5–35%). |
| Drop | **Automatic drop strobe REMOVED from use (operator: "troppo random se non leggono la musica", 2026-09-19 night): `DROP_STROBE_ENABLED = False`** - the analyzer's drop rule fires at random on real music. Drops are now ignored by the lights (no strobe, no colour change). The manual strobe on F8 stays. Set the flag back to True to re-enable (6 flashes, 30 s cooldown). |
| On/off + level | **New hotkeys**: `PUPA_LUCI` (Show = on, Hide = off, fade) and `PUPA_LUCI_LIVELLO_1..3` (ceiling ×0.15/×0.50/×1.00 since 2026-09-19 night - operator asked for more difference between the levels; was ×0.35/×0.70/×1.00 - default 2). Both persist across restarts (read back from OBS). |
...
**Launchers (2026-09-19 night)**: Linux `./avvia_luci.sh [start|restart|stop|status]`, Windows `avvia_luci.bat [start|restart|stop|status]` (wraps `avvia_luci.ps1`). They check QLC+ (OS2L port 9996; Linux: offer to relaunch it with the project and `-p` if it is open without OS2L; Windows: QLC+ 5 at `C:\QLC+5\qlcplus-qml.exe -w -o <project>`, it has no `-p`), OBS (WebSocket 4455, only a warning), start `lights/pupa_luci.py` detached with output in `lights/logs/stdout.log`, wait for the first `[SUM]` line and print a verdict (`qlc=ok`). `stop` = clean shutdown, falling back to a forced kill + `pupa_luci.py --zero` (zeroes all channels, paced). Windows `start` was syntax/status-tested only (QLC+ 5 would open its GUI here); the Linux script was tested for real (start, already-running guard, status, stop, DMX back to 0). Manual run: `cd ~/Desktop/pupa && nohup python3 lights/pupa_luci.py > /tmp/luci.log 2>&1 < /dev/null & disown`. `--no-obs` = no hotkeys (default on, level 2). Stop: F12 (also stops `pupa.py`), or `./avvia_luci.sh stop`. **Gotcha fixed**: a background process started from a non-interactive shell inherits SIGINT as *ignored*, so Python never installs its Ctrl+C handler and `kill -INT` did nothing (the script's stop timed out); `pupa_luci.py` now installs explicit SIGINT/SIGTERM handlers that run the normal shutdown cascade.
...
2. `python3 lights/create_obs_sources.py` (dry-run), then `--apply`. **Operator binds real keys** in OBS Settings → Hotkeys: Show of `PUPA_LUCI` (+ optional Hide on another key) and Show of each `PUPA_LUCI_LIVELLO_n`. Keys chosen and bound 2026-09-19: F5/Shift+F5 on/off, Shift+1/2/3 levels (Ctrl/Alt+F-key don't work on this setup). Verify in the scene collection JSON (`tso_backup_before_scene_removal.json`) as done on 2026-09-18.
...
- A musically-grounded automatic strobe (the analyzer's drop rule is too random); until then only F8.
