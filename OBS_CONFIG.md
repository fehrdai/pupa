...
| `_B` | secondary scene, shared pool across **all** `_A` (any scene_A can show any scene_B, except image-with-image pairs - see "Images are never paired with images" below) | at least one |
...
**Images are never paired with images (2026-09-20).** A scene *contains images* if it holds (directly or through a nested scene) a source whose kind is `slideshow` or `image_source` - again by content, not by name; `pupa.py` computes this at startup (`scene_discovery.scenes_with_images`, includes hand-written B scenes without the `_B` suffix like `slide`) and hands it to `brain.set_image_scenes()`. `brain._select_b_scene()` never gives a scene_A with images a scene_B with images (and vice versa); if no image-free B exists the rule yields instead of leaving the A without a B (logged as `[IMMAGINI]`). The one-shot "`slide` is the first B at startup" override (2026-07-16) now also respects the rule. On the current rig: `futureflash_A`, `montezuma_A` (video + slideshow) and `slide` (images only) are the image scenes, so `slide` only pairs with `kusanagi_A`/`mri_A`. The console prints `[PUPA] Scene con immagini (mai accoppiate tra loro): [...]` at startup - check it after adding scenes.
...
| ~~`PUPA_LIGHTMODE_*`~~ | (deleted 2026-09-19) | ~~F5/F6/F7~~ | Removed from OBS with the old light code; F5/F6/F7 (and Shift+) are free again. |
...
| `PUPA_LUCI` (2026-09-19) | binary toggle | **F5** show, **Shift+F5** hide | Lights on (Show) / off (Hide, fade) - read by `lights/pupa_luci.py`, not `pupa.py`. Persists across restarts. |
| `PUPA_LUCI_LIVELLO_1`..`_3` (2026-09-19) | 3-way exclusive | **Shift+1 / Shift+2 / Shift+3** show | Lights intensity ceiling x0.15 / x0.50 / x1.00 (default 2; was 0.35/0.70/1.00 until 2026-09-19 night) - same "most recently shown wins" as CALM. Read by `lights/pupa_luci.py`. (Ctrl/Alt + function keys do NOT work on this OBS/keyboard setup; Shift + F-key and Ctrl + letter do.) |

**Lights redesign 2026-09-19**: the old light code is REMOVED from `pupa.py`/`brain.py` (commit `b5b3aaa`). The lights are the separate process `lights/pupa_luci.py` (see `LIGHTS_CONFIG.md`), which reads `PUPA_STROBE_WHITE` (F8), `PUPA_BLACKOUT` (F11), `PUPA_SHUTDOWN` (F12) and the two new sources above. `PUPA_LIGHTMODE_*` (F5/F6/F7) were DELETED from OBS (2026-09-19; note `remove_input` over the WebSocket returned success but did nothing - removing the scene item with `remove_scene_item` is what actually deleted them); `PUPA_SOLO_LUCI`/`PUPA_SOLO_MONITOR` (F9/F10) still act on the MONITORS only (`brain.forced_mode`: F9 = monitors off, F10 = monitors on) and no longer touch the lights; F11 blackout in `pupa.py` now only blacks the monitors/OBS (the lights black out on their own).
